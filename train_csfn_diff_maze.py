...
import utils
from core_csfn.csfn_diff import CSFN
...
while(True):    
    ...
    # ------------ Forward ------------
    net.zero_grad()
    loss = net(x_obs, x_query_gt, v_obs, v_query)
    rec = [float(loss.detach().cpu().numpy())]
    ...
    # ------------ Print Result ------------
    if steps % 100 == 0:
        print("[Ep %s/%s] loss: %f"%(str(steps), str(args.total_steps), rec[0]))
