...
def draw_query_maze(net, color, pose, obs_size=3, row_size=32, gen_size=10, shuffle=False, border=[1,4,3]):
    ...
    for it in range(gen_size):
        ...
        vsize = v_obs.shape[-1]
        with torch.no_grad():
            x_query_sample, x_diff_sample = net.sample(x_obs, v_obs, v_query, n_obs=obs_size)
            x_query_sample = x_query_sample.detach().permute(0,2,3,1).cpu().numpy()
            x_diff_sample = x_diff_sample.detach().permute(0,2,3,1).cpu().numpy()
...
while(True):    
    ...
    if args.stochastic_unit:
        x_query, kl_query, diff_loss = net(x_obs, v_obs, x_query_gt, v_query, n_obs=obs_size)
        lh_query = criterion(x_query, x_query_gt).mean()
        kl_query = torch.mean(torch.sum(kl_query, dim=[1,2,3]))
        loss_query = lh_query + args.kl_scale*kl_query + diff_loss
        rec = [float(loss_query.detach().cpu().numpy()), float(lh_query.detach().cpu().numpy()), float(kl_query.detach().cpu().numpy()), float(diff_loss.detach().cpu().numpy())]
    else:
        x_query, x_diff = net.sample(x_obs, v_obs, v_query, n_obs=obs_size)
