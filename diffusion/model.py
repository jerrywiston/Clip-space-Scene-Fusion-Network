...
class DiffusionModel(nn.Module):
    ...
    def forwardX(self, x, t):
        """
        Model is U-Net with added positional encodings and self-attention layers.
        """
        img_size = int(self.in_size ** (1/2))
        x1 = self.inc(x)
        x2 = self.down1(x1) + self.pos_encoding(t, 128, int(img_size/2))
        x3 = self.down2(x2) + self.pos_encoding(t, 256, int(img_size/4))
        x3 = self.sa1(x3)
        x4 = self.down3(x3) + self.pos_encoding(t, 256, int(img_size/8))
        x4 = self.sa2(x4)
        x = self.up1(x4, x3) + self.pos_encoding(t, 128, int(img_size/4))
        x = self.sa3(x)
        x = self.up2(x, x2) + self.pos_encoding(t, 64, int(img_size/2))
        x = self.up3(x, x1) + self.pos_encoding(t, 64, int(img_size))
        output = self.outc(x)
        return output
    ...
    def get_loss(self, batch, c):
        ...
        e_hat = self.forward(noise_imgs, c, ts.unsqueeze(-1).type(torch.float))
        loss = nn.functional.mse_loss(
            e_hat.reshape(-1, self.in_size), epsilons.reshape(-1, self.in_size)
        )
        return loss

    def denoise_sample(self, x, c, t):
        """
        ...
        with torch.no_grad():
            if t > 1:
                ...
                z = 0
            e_hat = self.forward(x, c, t.view(1, 1).repeat(x.shape[0], 1))
