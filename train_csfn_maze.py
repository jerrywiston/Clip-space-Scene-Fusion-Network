...
import utils
from core_csfn.csfn import CSFN
...
def draw_query_maze(net, color, pose, obs_size=3, row_size=32, gen_size=10, shuffle=False, border=[1,4,3]):
    ...
    for it in range(gen_size):
        ...
        vsize = v_obs.shape[-1]
        with torch.no_grad():
            x_query_sample = net(x_obs, v_obs, v_query)
...
while(True):    
    ...
    # ------------ Forward ------------
    net.zero_grad()
    x_query = net(x_obs, v_obs, v_query)
    ...
    # ------------ Print Result ------------
    if steps % 100 == 0:
        print("[Ep %s/%s] loss_q: %f"%(str(steps), str(args.total_steps), rec[0]))
