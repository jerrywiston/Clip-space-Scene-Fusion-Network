...
import utils
from core_csfn.csfn_generative import CSFN
...
def draw_query_maze(net, color, pose, obs_size=3, row_size=32, gen_size=10, shuffle=False, border=[1,4,3]):
    ...
    for it in range(gen_size):
        ...
        vsize = v_obs.shape[-1]
        with torch.no_grad():
            x_query_sample = net.sample(x_obs, v_obs, v_query)
...
while(True):    
    ...
    # ------------ Forward ------------
    net.zero_grad()
    x_query, loss, kl_loss, rec_loss = net(x_obs, x_query_gt, v_obs, v_query)
    rec = [float(loss.detach().cpu().numpy()), float(rec_loss.detach().cpu().numpy()), float(kl_loss.detach().cpu().numpy())]
    ...
    # ------------ Print Result ------------
    if steps % 100 == 0:
        print("[Ep %s/%s] loss: %f, rec_loss: %f, kl_loss: %f"%(str(steps), str(args.total_steps), rec[0], rec[1], rec[2]))
